exports.run = async (bot, msg) => {
	if (msg.author.id !== bot.config.botCreatorID) {
		if (!msg.member.hasPermission('MANAGE_EMOJIS')) return msg.channel.send(`<:redx:411978781226696705> you are missing permissions \`Manage Emojis\`!`);
	}
	...
	const splitString = (string, prepend = '', append = '') => {
		...
		const splitIndex = string.lastIndexOf('>', MESSAGE_CHAR_LIMIT - prepend.length - append.length); // string.lastIndexOf('\n',
