...
exports.run = async (bot, msg, args) => {
	try {
		if (args.length <= 0) return msg.channel.send(`<:redx:411978781226696705> Please provide a city.`).catch(err => console.error);

		const city = args.join(' ');
		const res = await got(makeURL(city), { json: true });

		if (!res || !res.body || !res.body.query || !res.body.query.results || !res.body.query.results.channel) return msg.channel.send(`<:redx:411978781226696705> Failed to load weather info!`).catch(err => console.error);
...
exports.info = {
	...
	examples: [
		'forecast NC',
		'forecast North Caroline',
		'forecast Raleigh'
	],
	description: 'Shows the 10 day forecast for a location.'
};
