exports.run = async (bot, msg, args) => {
	try {
		...
		(await msg.channel.send({ embed: ({ title: `<a:loading:414954381176340480> Purging \`${parseFloat(args[0].toString().replace(/^[-]/g,'')) > 100 ? 100 : parseFloat(args[0].toString().replace(/^[-]/g,''))}\` messages...` })}).then(async(msg) => {
			await msg.channel.messages.fetch({ limit: parseFloat(args[0].toString().replace(/^[-]/g,'')) > 100 ? 100 : parseFloat(args[0].toString().replace(/^[-]/g,'')) }).then(async(messages) => {
