exports.run = async (bot, msg, args) => {
	if (msg.author.id !== bot.config.botCreatorID) {
		if (!msg.member.hasPermission('MANAGE_SERVER') || !msg.member.hasPermission('MANAGE_MESSAGES')) return msg.channel.send(`<:redx:411978781226696705> You are missing the permissions \`Manage Server\` or \`Manage Messages\`!`).catch(console.error);
	}
    if (args.length < 1) {
