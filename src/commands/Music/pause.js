	...
	try {
		...
		if (dispatcher) {
			switch (dispatcher.pause) {
				case false:
					dispatcher.pause();
					return msg.channel.send(`<:check:411976443522711552> Playback paused.`);
				default:
					return msg.channel.send(`<:redx:411978781226696705> Playback already paused!`).catch(err => console.error);
			}
		}
	} catch (err) {
