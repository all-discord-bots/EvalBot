exports.run = async (bot, msg, args) => {
	try {
		...
		if (!bot.voice.connections.has(msg.guild.id)) return msg.channel.send('<:redx:411978781226696705> No music is being played');
		...
		if ((!bot.voice.connections.get(msg.guild.id).player.dispatcher) || (bot.voice.connections.get(msg.guild.id).player.dispatcher && !bot.voice.connections.get(msg.guild.id).player.dispatcher.paused)) return msg.channel.send('<:redx:411978781226696705> Playback is not paused!');
		try {
			bot.voice.connections.get(msg.guild.id).player.dispatcher.resume();
