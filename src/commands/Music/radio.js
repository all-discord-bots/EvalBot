...
//	};
	...
	let filteredbuiltinradio = radiostationsqueue.map(list => list.toString()).filter(list => list.startsWith(getarg.toString()));
	if (getarg.length <= 4 && filteredbuiltinradio.length < 1) return msg.channel.send(`<:redx:411978781226696705> You must provide a valid stream`).catch(console.error);
	let queuethis;
