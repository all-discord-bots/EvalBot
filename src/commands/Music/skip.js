...
exports.run = async (bot, msg, args) => {
	try {
		...
		if (voiceConnection === null || !musicqueue[msg.guild.id] || musicqueue[msg.guild.id] && musicqueue[msg.guild.id]['music'].length < 1) return msg.channel.send(`<:redx:411978781226696705> There is no audio being played.`).catch(err => console.error);
