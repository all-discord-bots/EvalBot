...
exports.run = async (bot, msg, args) => {
	try {
		...
		if ((!bot.voice.connections.has(msg.guild.id) || !fetched_queue) || (fetched_queue && fetched_queue.queue.length < 1)) return msg.channel.send('<:redx:411978781226696705> There is no audio being played.');
		...
		try {
			if (bot.voice.connections.has(msg.guild.id) && bot.voice.connections.get(msg.guild.id).paused) bot.voice.connections.get(msg.guild.id).player.dispatcher.resume();
